...
splitted = infy['Date'].str.split('-', expand=True)
 
infy['month'] = splitted[1].astype('int')
infy['year'] = splitted[0].astype('int')
infy['day'] = splitted[2].astype('int')
...
plt.figure(figsize=(16,6))
plt.title('Vol history')
plt.plot(infy['Volume'])
plt.xlabel('Date', fontsize=18)
...
infy[infy['Close'] == infy['Adj Close']].shape
...
print('Test MAPE:', r ound(get_mape(Y_test,Y_test_pred), 2)) 
